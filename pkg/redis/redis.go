...
func (rds *Client) Metric(ctx context.Context, start time.Time) {
	...
	if duration.Milliseconds() >= conf.AppConfig.Redis.RedisWarnTime {
		...
		logger.Warn(ctx, consts.METRIC_REDIS, zap.Int64("duration", duration.Milliseconds()),
			zap.Any("function", runtime.FuncForPC(pc).Name()), zap.Any("file", fmt.Sprintf("%s:%d", file, line)))
	}
}
